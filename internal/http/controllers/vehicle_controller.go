...
// GetMyVehicleAnalytics returns analytics for a user's specific vehicle
func (vc *VehicleController) GetMyVehicleAnalytics(c *gin.Context) {
	// ... existing code ...
}
...
func (vc *VehicleController) RevokeVehicleShare(c *gin.Context) {
	...
	// Delete the share
	if err := db.GetDB().Unscoped().Delete(&shareToRevoke).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false,
